...
3. **Filter & Search Emails**:
   - Use the category buttons (`BL_COMPARISON`, `SI_REQUEST`, `INVOICE_QUERY`, `GENERAL`, `SPAM`) to filter by intent.
   - Filter by verification status: **`All`**, **`Mismatch (46)`**, **`Needs Review (20)`**, or **`OK (154)`**.
   - Use the search bar to find emails by subject, sender, or email ID (e.g., `email_004`).
4. **Inspect Discrepancies**:
   - Click on any email with a red **`MISMATCH`** badge (e.g., `email_004`).
   - The side-by-side drawer opens showing the **Shipment Field Comparison** table with conflicting values highlighted in red (e.g., `SI: EAST BRIGHT FZ-LLC / BL: UAB NOVAKOPA`).
   - Click the **`Source Documents Text`** tab to view the raw extracted text of the Shipping Instruction (SI) and draft Bill of Lading (BL) side-by-side.
5. **Human-in-the-Loop Review & Escalation**:
   - Click on any email with a yellow **`NEEDS_REVIEW`** badge (e.g., `email_501`, `email_512`, `email_516`).
   - View the escalation reason code (`wrong_doc_type`, `unreadable`, `missing_attachment`, `missing_value`) and evidence context.
   - Use the operational action buttons: **`Confirm Rejection`**, **`Approve As Exception`**, or **`Request Re-upload`** to resolve the item.
6. **View Official Scoreboard**:
   - Click the **`🏆 View Scoreboard`** button in the top right to inspect the live benchmark evaluation (**1.0000 / 100% Score**).
...
├── DEMO_GUIDE.md             # Live demonstration & video presentation guide
